            ...
            <ul class="list-group">
                <?php foreach ($resume_list as $item): ?>
                    <li id="rlist-<?= $item['id'] ?>" class="list-group-item list-group-item-action">
                        <a href="resume_detail.php?id=<?= $item['id'] ?>" class="btn btn-light"
                           target="_blank"><?= $item['title'] ?></a>

                        <a href="resume_detail.php?id=<?= $item['id'] ?>" target="_blank"><img src="image/launch.png"
                                                                                               alt="查看" ></a>
                        <a href="resume_modify.php?id=<?= $item['id'] ?>"><img src="image/create.png" alt="编辑"></a>
                        <a href="javascript:confirm_delete('<?= $item['id'] ?>');void(0);"><img src="image/clear.png"
                                                                                                alt="删除"></a>
                    </li>
                <?php endforeach; ?>
            </ul>
        <?php endif; ?>
        <p style="margin-top: 16px;"><a href="resume_add.php" class="resume_add btn btn-light"><img src="image/add.png" alt="添加简历"> 添加简历</a></p>
    </div>
